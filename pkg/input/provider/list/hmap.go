...
func (i *ListInputProvider) Set(executionId string, value string) {
	...
	if i.ipOptions.ScanAllIPs {
		...
		if dialers == nil {
			gologger.Warning().Msgf("scanAllIps: dialers not found for executionId %v, reverting to default", executionId)
		} else {
	...
	if i.ipOptions.IPV6 {
		dialers := protocolstate.GetDialersWithId(executionId)
		if dialers == nil {
			gologger.Warning().Msgf("target does not have ipv6 address falling back to ipv4: dialers not found for executionId %v", executionId)
		} else {
			dnsData, err := dialers.Fastdialer.GetDNSData(urlx.Hostname())
