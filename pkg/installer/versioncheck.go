...
// NucleiVersionCheck checks for the latest Nuclei and template versions. It
// returns the current remote ignore-file hash without persisting it.
func NucleiVersionCheck() (string, error) {
	return doVersionCheck(false)
}

var sdkVersionCheck = sync.OnceValues(func() (string, error) {
	return doVersionCheck(true)
})

// NucleiSDKVersionCheck checks for the latest versions in SDK mode. It returns
// the same result for every call in the process.
func NucleiSDKVersionCheck() (string, error) {
...
func UpdateIgnoreFile() error {
	...
	if resp.StatusCode != http.StatusOK {
		return fmt.Errorf("download .nuclei-ignore file: unexpected HTTP status %s", resp.Status)
	}
	...
	if err := config.DefaultConfig.WriteActiveIgnoreFile(bin); err != nil {
		return err
	}
...
func doVersionCheck(isSDK bool) (string, error) {
	...
	if err := config.DefaultConfig.WriteVersionCheckData("", nucleiversion, templateversion); err != nil {
		return "", err
	}

	return pdtmResp.IgnoreHash, nil
}
