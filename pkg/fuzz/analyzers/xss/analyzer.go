...
func (a *Analyzer) Analyze(options *analyzers.Options) (bool, string, error) {
	...
	// Fix for issue #7086 - Reflection detection should be case-insensitive
	if !strings.Contains(strings.ToLower(body), strings.ToLower(canary)) {
		return false, "", nil
	}

	// Detect character survival (case-insensitive)
	// Fix for issue #7086 - Character survival detection should be case-insensitive
...
// Uses case-insensitive matching for the canary
// Fix for issue #7086 - Character survival detection should be case-insensitive
func detectCharacterSurvival(body string, canary string) CharacterSet {
	...
	return CharacterSet{
		...
		ForwardSlash: strings.Contains(bodyLower, strings.ToLower(canary+canaryChars)), // full canary+chars survived
	}
}
