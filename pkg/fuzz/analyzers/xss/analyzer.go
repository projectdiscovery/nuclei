...
// extractCanaryFromValue attempts to extract the canary from a fuzz-generated value
// by looking for the known "nuclei" prefix followed by 8 random chars before canaryChars.
...
func (a *Analyzer) Analyze(options *analyzers.Options) (bool, string, error) {
	// Prefer canary extraction from the final generated fuzz value
	canary := extractCanaryFromValue(options.FuzzGenerated.Value)
	if canary == "" {
		// Fall back to shared analyzer parameters
	...
	// Check if canary is reflected (case-insensitive to handle server-side case transforms)
...
func (a *Analyzer) replayAndVerify(options *analyzers.Options, payload string, reflection *ReflectionInfo) (bool, string, error) {
	...
	// Always restore original value, even if rebuild/send fails
	defer func() {
