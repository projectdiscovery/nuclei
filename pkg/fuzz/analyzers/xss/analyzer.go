...
// canaryPattern matches canaries generated by generateCanary (nuclei + 8 alphanums).
var canaryPattern = regexp.MustCompile(`nuclei[a-zA-Z0-9]{8}`)
...
func (a *Analyzer) Analyze(options *analyzers.Options) (bool, string, error) {
	// Extract the canary from the per-request generated value to avoid
	// shared-state issues when AnalyzerParameters is reused across requests.
...
// extractCanaryFromValue extracts the canary string from a per-request
// generated value. This avoids reliance on the shared AnalyzerParameters map
// which may be overwritten by concurrent requests.
func extractCanaryFromValue(value string) string {
	return canaryPattern.FindString(value)
}

// detectCharacterSurvival checks which XSS-critical characters survived
// server-side encoding. Each character is checked independently so that
// encoding of one character does not mask survival of another.
