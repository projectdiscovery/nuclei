...
func (a *Analyzer) Analyze(options *analyzers.Options) (bool, string, error) {
	...
	// Check if canary is reflected at all (case-insensitive: servers may
	// normalise case in attribute values or text nodes).
	...
	// No payloads available is a non-match, not an error: return false, nil
	// rather than surfacing noisy warnings through the HTTP analyzer call path.
...
func (a *Analyzer) replayAndVerify(options *analyzers.Options, payload string, reflection *ReflectionInfo) (bool, string, error) {
	...
	// Restore original value immediately after SetValue to ensure cleanup even if Rebuild fails
	defer func() {
