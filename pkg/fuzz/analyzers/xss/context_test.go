...
func TestAnalyzeContext(t *testing.T) {
	tests := []struct {
		...
		{
			name:         "vbscript URI in href attribute",
			html:         `<a href="vbscript:MsgBox(nucleiXSScanary)">click</a>`,
			reflection:   "nucleiXSScanary",
			expected:     ContextScript,
			expectedAttr: "href",
		},
		{
			name:         "data URI in iframe src",
			html:         `<iframe src="data:text/html,<script>nucleiXSScanary</script>"></iframe>`,
			reflection:   "nucleiXSScanary",
			expected:     ContextScript,
			expectedAttr: "src",
		},
		...
		{
			name:         "module script block is executable",
			html:         `<script type="module">nucleiXSScanary</script>`,
			reflection:   "nucleiXSScanary",
			expected:     ContextScript,
			expectedAttr: "",
		},
		{
			...
			html:         `<script type="application/javascript">nucleiXSScanary</script>`,
			reflection:   "nucleiXSScanary",
			expected:     ContextScript,
			expectedAttr: "",
		},
