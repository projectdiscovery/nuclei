...
func TestPathNumericFuzzing(t *testing.T) {
	...
	require.NoError(t, err, "could not parse path")
	...
	require.NoError(t, err, "could not iterate")
	...
	require.NoError(t, err, "should be able to fuzz numeric path segment")

	// Rebuild the request with the fuzzed value
	rebuilt, err := path.Rebuild()
	require.NoError(t, err, "could not rebuild request")
