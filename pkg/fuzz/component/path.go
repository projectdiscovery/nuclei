...
func (q *Path) Rebuild() (*retryablehttp.Request, error) {
	...
	originalSegments := strings.Split(q.req.Path, "/")

	// Create a new slice to hold the rebuilt segments
	rebuiltSegments := make([]string, 0, len(originalSegments))

	// Add the first empty segment (from leading "/")
	if len(originalSegments) > 0 && originalSegments[0] == "" {
		rebuiltSegments = append(rebuiltSegments, "")
	}

	// Process each segment
	segmentIndex := 1 // 1-based indexing for our stored values
	for i := 1; i < len(originalSegments); i++ {
		originalSegment := originalSegments[i]
		if originalSegment == "" {
			// Skip empty segments
			continue
		}
