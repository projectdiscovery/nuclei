...
func (jsp *JsPool) executeWithRuntime(runtime *goja.Runtime, p *goja.Program, args *ExecuteArgs, opts *ExecuteOptions) (result goja.Value, err error) {
	defer func() {
...
func (jsp *JsPool) ExecuteProgram(p *goja.Program, args *ExecuteArgs, opts *ExecuteOptions) (result goja.Value, err error) {
	if opts.Source == nil {
		// not-recommended anymore
		return jsp.executeWithoutPooling(p, args, opts)
	}
	if !stringsutil.ContainsAny(*opts.Source, exportAsToken, exportToken) {
		// not-recommended anymore
		return jsp.executeWithoutPooling(p, args, opts)
	}
	return jsp.executeWithPoolingProgram(p, args, opts)
}

// executes the actual js program
func (jsp *JsPool) executeWithPoolingProgram(p *goja.Program, args *ExecuteArgs, opts *ExecuteOptions) (result goja.Value, err error) {
	...
	jsp.pooljsc.Add()
	defer jsp.pooljsc.Done()

	runtime, err := jsp.sizedGojaPool.Get(context.TODO())
	if err != nil {
		return nil, err
	}

	defer jsp.sizedGojaPool.Put(runtime)
	...
	val, err := jsp.executeWithRuntime(runtime, p, args, opts)
	if err != nil {
		return nil, err
	}
	if val.Export() != nil {
