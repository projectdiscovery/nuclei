...
func CreateReportDescription(event *output.ResultEvent, formatter ResultFormatter, omitRaw bool) string {
	...
	if event.Interaction != nil {
		...
		if event.Interaction.QType != "" {
			fmt.Fprintf(builder, " (%s)", event.Interaction.QType)
		}
	...
	fmt.Fprintf(builder, "Generated by %s", formatter.CreateLink("Nuclei "+config.Version, "https://github.com/projectdiscovery/nuclei"))
	data := builder.String()
	return data
}
