...
## Specifying Targets with **Nuclei**

When executing Nuclei, you can specify the targets (URLs or hosts) in several ways, ensuring flexibility based on your use-case requirements.

### 1. **Direct Target Specification** (`-u/-target`)

The simplest way to run Nuclei is by providing a single URL or host using the `-u` or `-target` flag:

```sh
nuclei -u https://example.com
```

Multiple URLs or hosts can be scanned by passing them as a comma-separated list:

```sh
nuclei -u https://example.com,http://testsite.com
```

### 2. **List of Targets** (`-l/-list`)

For scanning multiple targets, you can provide a file containing a list of URLs or hosts, one per line, using the `-l` or `-list` option:

```sh
nuclei -list targets.txt
```

### 3. **Excluding Specific Targets** (`-etg/-exclude-targets`)

There may be scenarios where you want to exclude certain IPs, CIDRs, or hostnames from being scanned. This can be achieved using the `-etg` or `-exclude-targets` flag:

```sh
nuclei -list targets.txt -etg 1.1.1.1,exclude.example.com
```

**NOTE**: 
- The exclude target process takes the input per line as provided without any alterations.
- Hostname resolutions are not performed to exclude IPs.


### 4. **Resuming a Scan** (`-resume`)

If a scan was previously interrupted, you can resume it using a configuration file:

```sh
nuclei -resume resume.cfg
```

Note: When using this option, clustering will be disabled.

### 5. **Scanning All Associated IPs** (`-sa/-scan-all-ips`)

To scan all IP addresses associated with a given DNS record:

```sh
nuclei -u example.com -sa
```

### 6. **IP Version Specification** (`-iv/-ip-version`)

By default, Nuclei scans using IP version 4. However, you can specify which IP version (4 or 6) you want to scan using the `-iv` or `-ip-version` option:
