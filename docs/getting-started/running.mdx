...
### 3. **Excluding Specific Targets** (`-eh/-exclude-hosts`)

There may be scenarios where you want to exclude certain IPs, CIDRs, or hostnames from being scanned. This can be achieved using the `-eh` or `-exclude-hosts` flag:

```sh
nuclei -list targets.txt -eh 1.1.1.1,exclude.example.com
```
