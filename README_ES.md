...
RATE-LIMIT:
   -rl, -rate-limit int               número máximo de peticiones a enviar por segundo (se ignora cuando -rate-limit-host está activo) (por defecto 150)
   -rld, -rate-limit-duration value   intervalo/ventana de recarga para -rate-limit (por defecto 1s)
   -rlm, -rate-limit-minute int       número máximo de peticiones a enviar por minuto (OBSOLETO)
   -rlh, -rate-limit-host int         número máximo de peticiones por host por rate-limit-host-duration (0 = desactivado, tiene prioridad sobre -rate-limit)
   ...
   -prc, -probe-concurrency int       concurrencia de sondas http con httpx (por defecto 50)
