...
func (request *Request) Compile(options *protocols.ExecuterOptions) error {
	...
	if len(request.Payloads) > 0 {
		var err error
		request.generator, err = generators.New(request.Payloads, request.AttackType.Value, options.TemplatePath, options.Options.TemplatesDirectory, options.Options.NoSandbox, options.Catalog, options.Options.AttackType)
		if err != nil {
