...
func (request *Request) ExecuteWithResults(input string, dynamicValues, previous output.InternalEvent, callback protocols.OutputEventCallback) error {
	...
	debugEvent := output.DebugEvent{Request: query, Response: jsonDataString}

	event := eventcreator.CreateEvent(request, data, debugEvent, request.options.Options.Debug || request.options.Options.DebugResponse)
