...
func TestTagBasedFilter(t *testing.T) {
	...
	t.Run("match-type", func(t *testing.T) {
		...
		matched, _ := filter.Match([]string{"fuzz"}, []string{"pdteam"}, severity.High, nil, "http")
		require.True(t, matched, "could not get correct match")
	})
	t.Run("match-exclude-type", func(t *testing.T) {
		filter := New(&Config{
			ExcludeTypes: []string{"http"},
		})
		matched, _ := filter.Match([]string{"fuzz"}, []string{"pdteam"}, severity.High, nil, "dns")
		require.True(t, matched, "could not get correct match")

		matched, _ = filter.Match([]string{"fuzz"}, []string{"pdteam"}, severity.Low, nil, "http")
		require.False(t, matched, "could not get correct match")
	})
}
