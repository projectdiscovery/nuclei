...
const (
	missingMandatoryField = "mandatory '%s' field is missing"
	invalidFieldFormat    = "invalid field format for '%s' (allowed format is %s)"
)
...
func validateTemplateFields(template *templates.Template) error {
	...
	if utils.IsBlank(info.Name) {
		errors = append(errors, fmt.Sprintf(missingMandatoryField, "name"))
	}

	if info.Authors.IsEmpty() {
		errors = append(errors, fmt.Sprintf(missingMandatoryField, "author"))
	}

	if template.ID == "" {
		errors = append(errors, fmt.Sprintf(missingMandatoryField, "id"))
	} else if !templateIDRegexp.MatchString(template.ID) {
		errors = append(errors, fmt.Sprintf(invalidFieldFormat, "id", templateIDRegexp.String()))
	}
