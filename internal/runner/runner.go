...
func (r *Runner) RunEnumeration() error {
	...
	// prefetch secrets if enabled or if auth provider has dynamic secrets
	// (templates that must execute before the main scan, e.g. login flows)
	if executorOpts.AuthProvider != nil && (r.options.PreFetchSecrets || len(executorOpts.AuthProvider.GetTemplatePaths()) > 0) {
