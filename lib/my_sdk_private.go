...
import (
	...
	"github.com/secoba/nuclei/v3/internal/runner"
	...
	nucleiUtils "github.com/secoba/nuclei/v3/pkg/utils"
)

func (e *NucleiEngine) applyRequiredDefaults2(ctx context.Context) {
...
// init
func (e *NucleiEngine) initMy(ctx context.Context) error {
	if e.opts.Verbose {
	...
	httpxOptions.RandomAgent = true
	if len(e.opts.Proxy) > 0 {
		if strings.HasPrefix(e.opts.Proxy[0], "socks5://") {
			httpxOptions.SocksProxy = e.opts.Proxy[0]
		} else {
			httpxOptions.HTTPProxy = e.opts.Proxy[0]
		}
	}
